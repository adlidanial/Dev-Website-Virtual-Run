<!DOCTYPE html>
<html>

<head>
    <?php include '../app/includes/header.php'; ?>
    <title>Parameter - Virtual Run</title>
</head>

<body style="background: var(--light);">
<?php include '../app/includes/admin-navbar.php'; ?>
    <main class="page contact-page">
        <section class="portfolio-block contact">
            <div class="container">
                <div class="heading">
                    <h2>KEMASKINI PARAMETER</h2>
                </div>
                <?php foreach($data["parameter"] as $row){ ?>
                <form class="bg-white" method="post" action="//<?php echo URLROOT; ?>/admin/kemaskiniparameter/<?php echo $row->PK_ID; ?>">
                    <div class="form-group">
                        <label>Nama Kumpulan</label>
                        <input class="form-control d-none" type="text" name="id" value="<?php echo $row->PK_ID; ?>">
                        <select class="form-control" name="kumpulan">
                                <option value="" selected disabled>Sila Pilih</option>
                                <?php
                                foreach($data["kumpulan"] as $kumpulan)
                                    echo "<option value='$kumpulan->PK_ID'" . ($row->FK_ID_NAMA_KUMPULAN == $kumpulan->PK_ID ? "selected" : "").">$kumpulan->NAMA_KUMPULAN</option>";
                                ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Nama Parameter</label><span style="color:red">&#42;</span>
                        <input class="form-control" type="text" name="parameter" value="<?php echo $row->NAMA_PARAMETER; ?>">
                    </div>
                    <div class="form-group">
                        <label>Status</label><span style="color:red">&#42;</span>
                        <select class="form-control" name="status">
                                <option value="" selected disabled>Sila Pilih</option>
                                <option value="Y" <?php if($row->STATUS == 'Y') echo "selected"; ?> >Aktif</option>
                                <option value="N" <?php if($row->STATUS == 'N') echo "selected"; ?> >Tidak Aktif</option>
                        </select>
                    </div>
                    <a class="btn btn-secondary border rounded" href="../senaraiparameter" type="button">Kembali</a>
                    <button class="btn btn-primary border rounded float-right" name="btnSimpan" type="submit">Simpan</button>
                </form>
                <?php } ?>
            </div>
        </section>
    </main>
    <?php include '../app/includes/admin-footer.php'; ?>
</body>

</html>